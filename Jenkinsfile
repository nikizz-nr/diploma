            ...
            steps {
                container('k8s-control') {
                    script {
                        ...
                        if (env.BRANCH_NAME == 'main') {
                            sh 'helm -n staging install diploma nhlstats --set image=${env.ECR_REGISTRY}/nhlstats'
                        }
                        if (env.BRANCH_NAME == 'production') {
                            sh 'helm -n production install diploma nhlstats --set image=${env.ECR_REGISTRY}/nhlstats --set tag=stable --set nodeport=32221'
                        }
                    }
                }
            }
